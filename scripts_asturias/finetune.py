...
data_asturias_path = paths['data_asturias']
...
# Load predictand
predictand_filename = f'{data_asturias_path}/{var_target}/day/*.nc'
predictand = xr.open_mfdataset(predictand_filename).load()
...
# Stack and filter the predictand data
...
# Name the model
model_name = f'deepESD_grid_asturias_{training_routine}_{var_target}_ens{num_ensemble}'
...
if var_target == 'pr':
    ...
    model = dl_model(x_shape=x_train_stand_arr.shape,
                     ...
                     filters_last_conv=1,
                     stochastic=False)
elif var_target in ('tasmin', 'tas', 'tasmax'):
...
if var_target in ('tasmin', 'tas', 'tasmax'):
...
for period in periods_fut:
    ...
    for metric in metrics:
        ccs_values = metrics_ccs.compute_ccs(hist_data=pred_hist[[var_target]], fut_data=pred_fut_period[[var_target]],
                                             reduction_function=metrics[metric],
                                             relative=relative)
        ccs_values = xr.merge((ccs_values[[var_target]], # Add additional variables other than var_target (station_id)
                               pred_hist[[var for var in pred_hist.data_vars if var != var_target]]))
