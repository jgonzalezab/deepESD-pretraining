...
var_target = 'pr'
num_ensemble = 1
training_routine_list = ['raw', 'original', 'pretrained', 'pretrained_finetuning', 'original_grid_spain']
...
maps_routine_name = {'raw': 'Global Climate Model',
                     ...
                     'original_grid_spain': 'No pre-training (Spain)',
...
for period in periods_fut.keys():
    for routine in training_routine_list:
        ...
        # Set model name
        model_name = f'deepESD_grid_asturias_{routine}_{var_target}_ens{num_ensemble}'
        ...
        if routine == 'raw':
            ...
            data = data * mask
        elif routine == 'original_grid_spain':
            model_name_spain = f'deepESD_{var_target}_ens{num_ensemble}'
            file_name = f'{ccs_path}/{gcm_model_name}_{scenario}_{gcm_run}_{period}_{model_name_spain}_{metric}.nc'
            data = xr.open_dataset(file_name)
        else:
            file_name = f'{ccs_path}/{gcm_model_name}_{scenario}_{gcm_run}_{period}_{model_name}_{metric}.nc'
            data = xr.open_dataset(file_name)
            lat_range = (data['lat'].values[0], data['lat'].values[-1])
            lon_range = (data['lon'].values[0], data['lon'].values[-1])

        # Spatially subset
        if routine in ('original_grid_spain'):
            data = data.sel(lat=slice(*lat_range)).sel(lon=slice(*lon_range))
